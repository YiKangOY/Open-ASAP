'''
This file is adapted from https://github.com/lydiawunan/LOSTIN/blob/main/GNN-LSTM/read_graph_raw.py
'''
import pandas as pd
...
def read_csv_graph_raw(raw_dir, add_inverse_edge = False, additional_node_files = [], additional_edge_files = []):
    ...
    - The name should be {additional_node_file, additional_edge_file}.csv
    ...
    for additional_file in additional_node_files:
        ...
        if additional_file == 'node_species' and osp.exists(osp.join(raw_dir, 'species.csv')):
            os.rename(osp.join(raw_dir, 'species.csv'), osp.join(raw_dir, 'node_species.csv'))

        temp = pd.read_csv(osp.join(raw_dir, additional_file + '.csv'),  header = None).values
    ...
    try:
        io_feat = pd.read_csv(osp.join(raw_dir, 'io-node-list.csv'),  header = None).values
        if 'int' in str(io_feat.dtype):
            io_feat = io_feat.astype(np.int64)
        else:
            #float
            io_feat = io_feat.astype(np.float32)
    ...
    for num_node, num_edge in tqdm(zip(num_node_list, num_edge_list), total=len(num_node_list)):
        ...
        # graph['io_ratio'] = np.array(io_feat[graph_idx])
        graph_idx+=1
